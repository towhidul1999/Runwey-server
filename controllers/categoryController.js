...
const createCategory = async (req, res, next) => {
    try {
        ...
        if (req.files && req.files.categoryImage && req.files.categoryImage[0]) {
            categoryImage = `/public/image/${req.files.categoryImage[0].filename}`;
        }

        if (req.files && req.files.categoryVideo && req.files.categoryVideo[0]) {
            categoryVideo = `/public/content/${req.files.categoryVideo[0].filename}`;
        }
